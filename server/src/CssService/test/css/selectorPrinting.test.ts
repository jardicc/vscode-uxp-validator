...
import * as selectorPrinting from "../../../CssServiceOriginal/services/selectorPrinting";
import { TextDocument, MarkedString } from "../../../CssServiceOriginal/cssLanguageTypes";
...
function elementToString(element: selectorPrinting.Element): string {
	...
	const attributes = element.attributes && element.attributes.filter(a => a.name !== "name");
...
export function assertSelector(p: Parser, input: string, selectorName: string, expected: string): void {
	const selector = doParse(p, input, selectorName);
	assert(selector);
...
function assertElement(p: Parser, input: string, expected: { name: string; value?: string }[]): void {
	const node = p.internalParse(input, p._parseSimpleSelector)!;
	const actual = selectorPrinting.toElement(node);
...
function assertSelectorMarkdown(
	...
	const printedElement = selectorPrinter.selectorToMarkedString(selector!);
