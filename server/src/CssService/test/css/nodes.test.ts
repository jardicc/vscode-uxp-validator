...
suite("CSS - Nodes", () => {
	...
	test("Test Node", function () {
		...
		const child = new nodes.Node();
		node.adoptChild(child);

		c = 0;
		const expects = [node, child];
		node.accept((n: nodes.Node) => {
	...
	test("Test Adopting", function () {
		...
		const p1 = new nodes.Node();
		const p2 = new nodes.Node();
	...
	function stylesheet(input: string): nodes.Stylesheet {
		const parser = new Parser();
		const node = parser.internalParse(input, parser._parseStylesheet)!;
		return node;
	}
	...
	test("Keyframe", function () {
		function fn(input: string): nodes.Node {
			const parser = new Parser();
			const node = parser.internalParse(input, parser._parseKeyframe)!;
			return node;
		}
	...
	test("UnicodeRange", function () {
		function fn(input: string): nodes.Node {
			const parser = new Parser();
			const node = parser.internalParse(input, parser._parseFontFace)!;
			return node;
		}
