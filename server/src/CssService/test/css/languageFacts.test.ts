...
import { isColorValue, getColorValue, getBrowserLabel, colorFrom256RGB, colorFromHex, hexDigit, hslFromColor, HSLA, hwbFromColor, HWBA, colorFromHWB, colorFromHSL } from "../../../CssServiceOriginal/languageFacts/facts";
import { Parser } from "../../../CssServiceOriginal/parser/cssParser";
import * as nodes from "../../../CssServiceOriginal/parser/cssNodes";
import { TextDocument, Color } from "../../../CssServiceOriginal/cssLanguageTypes";
...
function assertHSLValue(actual: HSLA, expected: HSLA) {
	if (actual && expected) {
		...
		const sDiff = Math.abs((actual.s - expected.s) * 100);
		const lDiff = Math.abs((actual.l - expected.l) * 100);
		const aDiff = Math.abs((actual.a - expected.a) * 100);
...
function assertHWBValue(actual: HWBA, expected: HWBA) {
	if (actual && expected) {
		const hDiff = Math.abs(actual.h - expected.h);
		const wDiff = Math.abs((actual.w - expected.w) * 100);
		const bDiff = Math.abs((actual.b - expected.b) * 100);
		const aDiff = Math.abs((actual.a - expected.a) * 100);
...
suite("CSS - Language Facts", () => {
	...
	test("properties", function () {
		const alignLast = cssDataManager.getProperty("text-decoration-color");
		if (!alignLast) {
			assert.ok(alignLast);
			return;
		}
		...
		const r = alignLast.restrictions;
