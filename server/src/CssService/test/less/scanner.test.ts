...
import { TokenType } from "../../../CssServiceOriginal/parser/cssScanner";
import { LESSScanner } from "../../../CssServiceOriginal/parser/lessScanner";

function assertSingleToken(source: string, len: number, offset: number, text: string, type: TokenType): void {
	const scan = new LESSScanner();
	scan.setSource(source);
	const token = scan.scan();
