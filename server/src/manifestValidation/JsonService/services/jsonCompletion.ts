...
export class JSONCompletion {
	...
	public doComplete(document: TextDocument, position: Position, doc: Parser.JSONDocument, customSchema?:SchemaService.ResolvedSchema): Thenable<CompletionList> {
		...
		const collector: CompletionsCollector = {
			add: (suggestion: JSONCompletionItem) => {
				...
				if (!existing) {
					...
					if (!(existing as any).labelDetails) {
						(existing as any).labelDetails = (suggestion as any).labelDetails;
					}
				}
			},
			setAsIncomplete: () => {
		...
		return this.schemaService.getSchemaForResource(document.uri, doc).then((schema) => {
			...
			if (node && node.type === "object") {
				...
				if (schema) {
					...
					this.getPropertyCompletions(schema, doc, node, addValue, separatorAfter, collector, customSchema);
				} else {
	...
	private getPropertyCompletions(schema: SchemaService.ResolvedSchema, doc: Parser.JSONDocument, node: ASTNode, addValue: boolean, separatorAfter: string, collector: CompletionsCollector, customSchema?:SchemaService.ResolvedSchema): void {
		const matchingSchemas = doc.getMatchingSchemas(schema.schema, node.offset);

		const process = (s: Parser.IApplicableSchema) => {
		...
		if (customSchema) {
			...
			matchingSchemas.forEach(process);
		}
	...
	private addDefaultValueCompletions(schema: JSONSchema, separatorAfter: string, collector: CompletionsCollector, arrayDepth = 0): void {
		...
		if (isDefined(schema.default)) {
			...
			if (this.doesSupportsLabelDetails()) {
				(completionItem as any).labelDetails = { description: l10n.t("Default value") };
			} else {
